...
export default function EsgScreeningListPage() {
  ...
  return (
    <div className="border border-line-card bg-surface">
      <div className="flex flex-wrap items-baseline justify-between gap-2 px-5 pt-4 sm:px-7">
        <Link
        ...
        <Link
          href="/unified/reports/esg-screening/lists"
          className="font-mono text-[10px] tracking-label text-link transition-colors hover:text-link-hover"
        >
          배제리스트 관리 →
        </Link>
      </div>
      ...
      <div className="px-5 py-5 pb-10 sm:px-7">
        <div className="mb-6 flex flex-wrap items-end gap-3 border border-ink bg-surface-muted p-4">
          <Field id="es-year" label="연도" className="w-28">
            <Select value={year} onChange={(e) => setYear(Number(e.target.value))}>
              {YEARS.map((y) => <option key={y} value={y}>{y}</option>)}
            </Select>
          </Field>
          <Field id="es-month" label="월" className="w-20">
        ...
        {!api || isLoading ? (
          ...
          <EmptyState
            title="아직 생성된 ESG 스크리닝 보고서가 없습니다"
