...
- **알려진 잔여 위험(낮음)**: 클라이언트가 파싱에서 버린 행 수(`skipped`)를 요약에 못 싣는다 — 반환 타입이 `Pair`라 자리가 없어 로그로만 남는다. 지수가 KOSPI 하나뿐인 지금은 전 행이 버려지면 `saved=0`이라 500 분기가 잡지만, **지수가 둘 이상으로 늘면 하나만 형식이 깨졌을 때 `emptySeries`가 그걸 가린다.** 그때는 클라이언트 반환 타입에 `skipped`를 실어야 한다
