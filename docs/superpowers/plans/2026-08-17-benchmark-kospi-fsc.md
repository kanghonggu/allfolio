...
**3. `fltRt`가 앞의 0을 생략한다** — `.73`, `-.6`. **금 수집기(#178)와 똑같은 함정**이다. 등락률을 저장하진 않지만 파싱 단계에서 죽으면 행이 통째로 버려진다. `FscCommodityClient`의 `decimalOrNull`을 보고 같은 방어를 쓸 것.
...
3. `fltRt`가 `.05`·`-.89` 형식이어도 행이 안 버려진다
...
| 2 | `decimalOrNull`을 `BigDecimal(s)` 직접 호출로 | 3 |
