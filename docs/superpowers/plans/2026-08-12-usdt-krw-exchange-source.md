...
- [ ] **Step 3: 요청 경로를 검증하는 테스트 (계획 수정, 2026-08-12)**

> **원래 이 태스크는 컴파일 확인만 하려 했다. 그건 틀렸다.**
> Binance가 실패한 지점은 파싱이 아니라 **URL이었다** — `?symbol=USDTKRW`라는 존재하지 않는 심볼.
> 파서 테스트는 그런 버그를 원리적으로 잡지 못한다. 응답이 아예 오지 않으니까.
> 즉 Task 2·3의 테스트를 다 통과해도 **이번 사고와 같은 종류의 버그는 그대로 재발한다.**
> 그래서 소스가 **실제로 어떤 경로를 때리는지**를 못 박는 테스트를 둔다.

JDK 내장 `com.sun.net.httpserver.HttpServer`를 쓴다 — 새 의존성이 필요 없다
(`mockwebserver`는 이 프로젝트에 없다). 소스가 `baseUrl`을 생성자로 받으므로 로컬 스텁을 가리키면 된다.

Create: `allfolio-backend/backend-app/src/test/kotlin/com/allfolio/fx/exchange/ExchangeFxSourceTest.kt`
 ...
 *
 * Binance 클라이언트가 몇 달간 조용히 실패한 원인이 정확히 여기였다 — 파싱이 아니라
...
Run: `cd allfolio-backend && ./gradlew :backend-app:test --tests "com.allfolio.fx.exchange.ExchangeFxSourceTest"`
Expected: PASS (5 tests)

경로 단언이 실패하면 **테스트를 코드에 맞추지 말 것.** 실제 엔드포인트는 Task 9에서 확인한 값이다.

- [ ] **Step 5: Commit**

```bash
git add allfolio-backend/backend-app/src/main/kotlin/com/allfolio/fx/exchange/UpbitFxSource.kt allfolio-backend/backend-app/src/main/kotlin/com/allfolio/fx/exchange/BithumbFxSource.kt allfolio-backend/backend-app/src/test/kotlin/com/allfolio/fx/exchange/ExchangeFxSourceTest.kt
git commit -m "feat(af-99): Upbit·Bithumb HTTP 시세 소스 + 요청 경로 테스트"
```
