...
>
> **분리는 두 조각이 다 있어야 선다.** 환산 경로를 가르는 쪽(아래 "어댑터의 정규화를
> 갈라야 한다")은 이미 들어왔고, 라벨을 맞추는 이쪽이 마지막 조각이다. 코드를 읽는 사람이
> `CurrencyConverter`의 USD·USDT 분기만 보고 분리가 선다고 판단하면 안 된다 —
> 어떤 라벨이 저장돼 있느냐가 실제로 어느 분기로 가는지를 정한다.
>
> 하나은행 첫 수집 전에는 `getUsdToKrw()`의 default가 `getUsdtToKrw()`라 두 분기가 같은 값을
> 내므로 **숫자는 아직 안 바뀐다.** 수집이 돌면 그때부터 갈린다.
