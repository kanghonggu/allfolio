...
**GitHub은 공개 저장소에서 60일간 활동이 없으면 예약 워크플로를 자동으로 비활성화한다**(소유자에게 메일이 간다).
이 크론이 환율을 최신으로 유지하는 유일한 수단이라, 두 달쯤 조용한 기간이 생기면 수집이 소리 없이 멈춘다.
멈춘 걸 알아채는 방법은 `hana_fx_quote`의 최신 `base_date`를 보는 것뿐이다.

`--max-time`은 전체가 아니라 **시도 1회당** 상한이다. 그래서 워크플로의 `timeout-minutes`와
curl의 `--max-time`·`--retry`·`--retry-delay`는 따로 노는 값이 아니다 — 한쪽을 늘리면 다른 쪽도 봐야 한다.
