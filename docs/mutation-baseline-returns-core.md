...
# 사후 대조 — AF-199 (2026-09-10)

TWR·external flow 영역의 "안 잡힘" 9건에 테스트를 붙이고 **같은 변이를 다시 넣었다.**

| 변이 | 기준선 | AF-199 이후 | 잡는 테스트 |
| --- | --- | --- | --- |
| DPE-M1 외부현금흐름 조정 제거 | 🔴 안 잡힘 | ✅ **잡힘** | `DailyPerformanceEngineTest > 입금은 수익이 아니다` 외 2 |
| DPE-M2 기하 연결을 산술로 | 🔴 안 잡힘 | ✅ **잡힘** | `> 누적은 곱으로 잇는다` 외 2 |
| DPE-M3 전일 NAV 0 가드 무력화 | 🔴 안 잡힘 | ✅ **잡힘** | `> 전일 NAV가 0이면 계산하지 않는다` 외 1 |
| DPE-M4 알파 부호 반전 | 🔴 안 잡힘 | ✅ **잡힘** | `> 알파는 포트폴리오에서 벤치마크를 뺀 값이다` 외 1 |
| RRG-M1 출금 부호 소실 | 🔴 안 잡힘 | ✅ **잡힘** | `ReturnsReportGeneratorTest > withdrawal keeps its sign…` |
| RRG-M3 커버리지 앵커를 앞으로 | 🔴 안 잡힘 | ✅ **잡힘** | `ReturnsCalculatorEdgeTest > 앵커는 cutoff 이전 마지막 관측이다…` |
| CF-M3 금액 양수 검증 제거 | 🔴 안 잡힘 | ✅ **잡힘** | `CashFlowTest > 금액이 0 이하면 만들 수 없다` |
| CF-M5 통화 대문자화 제거 | 🔴 안 잡힘 | ✅ **잡힘** | `CashFlowTest > 통화는 대문자로 정규화한다` |
| **RC-M5** 연환산 365 → 360 | 🔴 안 잡힘 | ⚪ **등가 변이 — 잡을 수 없다** | 아래 |

## 🔴 RC-M5는 테스트로 잡을 수 없다 — 등가 변이다

`365`가 **두 곳**에 쓰인다: 순현재가치의 할인 지수(`days/365`)와 기간 환산 지수(`days/365`).
둘을 같이 바꾸면 **일 단위 성장률이 보존**되고, 공개 API가 내놓는 것은 기간 수익률뿐이라
차이가 사라진다.

수치로 확인했다 (기초 −1,000 · 14일째 −1,000 · 29일째 +2,200):

```javascript
basis=365: 연율=3.8245977792  기간수익률=0.133189103449
basis=360: 연율=3.7217026256  기간수익률=0.133189103449
```

**연율은 2.7% 달라지는데 기간 수익률은 소수 12자리까지 같다.** `PeriodReturns`는 연율을
노출하지 않으므로 어떤 단언으로도 구분할 수 없다.

→ **테스트를 만들지 않았다.** 억지로 붙이려면 내부 함수를 열거나 무의미한 단언을 넣어야
   하고, 둘 다 표를 오염시킨다. 연율을 밖으로 내보내게 되면 그때 잡을 수 있다.

→ 위험도도 재조정한다. 기준선에서 3군 11위였는데, **관측 불가이므로 사용자가 보는 숫자에
   영향이 없다.** 다만 코드로서는 여전히 이상하다 — 한쪽만 고치면 그때는 잡힌다.

## 남은 "안 잡힘" — AF-200 몫

`FRP-M1`(같은 날 타이브레이크) · `FRP-M2`(종료일 당일 제외) · `FRP-M4`(시작일 당일이 이전으로).
전부 `FifoRealizedPnlCalculator`의 기간 경계다.
...
# 사후 대조 — AF-200 (2026-09-10)

FIFO lot 영역에 남아 있던 "안 잡힘" 3건에 테스트를 붙이고 **같은 변이를 다시 넣었다.**
전부 `FifoRealizedPnlCalculator`의 기간 경계다. **운영 코드는 한 줄도 고치지 않았다.**

먼저 현재 main(`cc100e7`)에서 세 변이를 다시 걸어 **여전히 안 잡히는지 재확인**했다
(기준선은 `0dec46e` 측정이고 그 뒤 AF-199가 테스트를 늘렸다).
세 건 모두 전 모듈 `./gradlew test`가 **BUILD SUCCESSFUL** — 즉 그대로 안 잡히고 있었다.

| 변이 | 기준선 | main(cc100e7) 재측정 | AF-200 이후 | 잡는 테스트 (기대 → 실제) |
| --- | --- | --- | --- | --- |
| FRP-M1 같은 날 타이브레이크 소실 | 🔴 안 잡힘 | 🔴 안 잡힘 | ✅ **잡힘** | `> 같은 날 거래는 입력 목록 순서가 아니라 createdAt 순으로 소진된다` (2000 → 1000) |
| FRP-M2 종료일 당일 제외 | 🔴 안 잡힘 | 🔴 안 잡힘 | ✅ **잡힘** | `> 종료일 당일 매도는 그달 실현손익에 들어간다` (500 → 0) |
| FRP-M4 시작일 당일이 이전으로 | 🔴 안 잡힘 | 🔴 안 잡힘 | ✅ **잡힘** | `> 시작일 당일 매도는 전월이 아니라 그달 실현손익에 들어간다` (500 → 0) |

FRP-M3(기간 차분 소실)은 기준선대로 `> 이전월 매도는 당월에 포함되지 않는다`가 이미 잡는다 — 건드리지 않았다.

## FRP-M1은 목록 순서를 어긋나게 넣어야만 잡힌다

`sortedWith`는 **안정 정렬**이다. 같은 날 거래를 createdAt 순서 그대로 목록에 넣으면
타이브레이크를 지워도 결과가 같아 **변이가 살아남는다.** 그래서 테스트는 목록 순서를
일부러 createdAt 순서와 어긋나게 넣는다(200원 매수 → 매도 → 100원 매수, createdAt은 09·10·11시).
정렬이 createdAt를 보면 100원 lot이 먼저 소진돼 2000, 목록순이면 1000이다.

`StockTrade.create`는 `createdAt`을 `LocalDateTime.now()`로 박으므로 결정적이지 않다 —
테스트는 `StockTrade.reconstruct`로 createdAt을 고정한다.

## 경계 테스트는 "어느 달에 들어가는가"를 양쪽에서 문다

M2·M4 테스트는 같은 거래 목록으로 **두 기간을 계산해** 단언한다:
종료일(6/30) 매도는 6월에 500이고 7월에 0, 시작일(6/1) 매도는 6월에 500이고 5월에 0.
한쪽만 단언하면 "그달에 있다"는 알아도 "옆 달로 새지 않는다"는 모른다.

## 이 검증이 증명하지 못하는 것

변이가 잡힌다는 것은 **그 줄을 그 방향으로 망가뜨리면 빨개진다**까지다.
KDoc이 스스로 적어 둔 한계 — 같은 날 여러 체결의 createdAt 순서가 **실제 체결순이 아닐 수 있다**는
것 — 은 테스트로 메울 수 없다. 이건 데이터의 문제지 코드의 문제가 아니다.
