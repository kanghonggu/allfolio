...
FIFO lot 영역의 "안 잡힘" 3건. 전부 `FifoRealizedPnlCalculator`의 기간 경계다.

| 변이 | 기준선 | AF-200 이후 | 잡는 테스트 |
| --- | --- | --- | --- |
| FRP-M1 같은 날 타이브레이크 소실 | 🔴 안 잡힘 | ✅ **잡힘** | `FifoRealizedPnlCalculatorTest > 같은 날 거래는 입력 순서가 아니라 createdAt으로 정렬한다` |
| FRP-M2 종료일 당일 제외 | 🔴 안 잡힘 | ✅ **잡힘** | `> 기간 종료일 당일 매도도 당월에 포함한다` |
| FRP-M4 시작일 당일이 이전으로 | 🔴 안 잡힘 | ✅ **잡힘** | `> 기간 시작일 당일 매도는 당월분이다` |

## `CostBasisMethod` 분기도 덮었다

기준선이 "변이 지점 없음"으로 기록하면서 **분기 위치**를 후속 입력으로 남겼다
(`PositionCacheService.costBasis:147`, 참조 테스트 0건). `PositionCacheServiceCostBasisTest` 4건을 넣었다.

무는 것은 **조용한 폴백**이다 — `FIFO -> fifoCostBasis ?: averageCost`. 두 방식이 같은 값을
내는 데이터로 재면 분기를 통째로 바꿔도 통과하므로, **평단 150 · FIFO 원가 100**처럼
값이 갈리는 데이터로 잰다.

그 과정에서 하나 갈렸다: 레거시 캐시(수량만 있고 `lots`가 빈 데이터)는 매퍼가
`(avgCost, quantity)` 단일 lot으로 **합성**해서 두 방식이 같아진다. 그건 폴백이 아니라
합성의 결과이고, 폴백이 실제로 걸리는 자리는 **보유가 0인 포지션**뿐이다. 둘을 나눠 적었다.

## 최종 상태 — 31개 중 30개가 잡힌다
...
**AF-197 관문 규칙의 개방 조건** — *"AF-174의 테스트는 변이 검사를 통과해야 한다.
TWR/MWR·external flow·FIFO lot 각각에 대해 로직을 한 줄 망가뜨렸을 때 실제로 빨간지
확인하고 그 결과를 태스크에 기록한다"* — 이 문서가 그 기록이다.

**Tier 2-2를 열 것인가는 사람 판단이다**(AF-201).
